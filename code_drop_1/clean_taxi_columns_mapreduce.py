...
for y in xrange(2009,2017):
	for m in xrange(1,13):
		...
		taxis = zones_rdd.join(dat.select(['id','longitude','latitude']))

		taxis_d = taxis.withColumn('dist', (taxis.longitude -taxis.lon)**2 + (taxis.latitude -taxis.lat)**2 ).select('complaint_id','dist','taxi_zones_id')

		# taxis_d_fil= taxis_d.filter(taxis_d.complaint_id =='491504201')
		taxis_with_id=taxis_d.groupBy('complaint_id').min('dist').show()
